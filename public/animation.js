...
console.log("wip three-animation");
...
animInit = () => {
  ...
  function setupNavCards(parent) {
    ...
    const applyMaterial = async items => {
      try {
        return await items.map(
          item => new THREE.MeshBasicMaterial({ map: item })
        );
      } catch (err) {
    ...
    const IMGS = async () => {
      ...
      Promise.all([imgs, texture, material, geometry, withShadow]).then(
        function(values) {
  ...
  const navEnterHandler = e => {
    ...
    navAnim[key] = TweenMax.to(navCards[key].position, 1, {
      z: -20,
